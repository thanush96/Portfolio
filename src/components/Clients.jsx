...
const Clients = () => (
  <section className={`${styles.flexCenter} my-6 sm:my-8`}>
    <div className={`grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 gap-2 sm:gap-3 w-full px-2 sm:px-4`}>
      {skills.map((skill) => (
        <div key={skill.id} className={`flex flex-col items-center justify-center p-2 sm:p-3 rounded-[8px] bg-black-gradient-2 hover:scale-105 transition-all shadow-sm hover:shadow-md cursor-pointer`}>
